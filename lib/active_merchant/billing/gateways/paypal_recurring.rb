        ...
        xml.tag! 'SetExpressCheckoutReq', 'xmlns' => PAYPAL_NAMESPACE do
          xml.tag! 'SetExpressCheckoutRequest', 'xmlns:n2' => EBAY_NAMESPACE do
            ...
            xml.tag! 'n2:SetExpressCheckoutRequestDetails' do
              ...
              xml.tag! 'n2:OrderTotal', localized_amount(money).to_f.zero? ? localized_amount(100) : localized_amount(money), 'currencyID' => options[:currency] || currency(money)
              ...
              xml.tag! 'n2:InvoiceID', options[:invoice]
          ...
          xml.tag! 'DoExpressCheckoutPaymentRequest', 'xmlns:n2' => EBAY_NAMESPACE do
            ...
            xml.tag! 'n2:DoExpressCheckoutPaymentRequestDetails' do
              ...
              xml.tag! 'n2:PaymentDetails' do
                xml.tag! 'n2:OrderTotal', localized_amount(money), 'currencyID' => currency_code

                # All of the values must be included together and add up to the order total
                if [:subtotal, :shipping, :handling, :tax].all?{ |o| options.has_key?(o) }
                  xml.tag! 'n2:ItemTotal', localized_amount(options[:subtotal]), 'currencyID' => currency_code
                  xml.tag! 'n2:ShippingTotal', localized_amount(options[:shipping]),'currencyID' => currency_code
                  xml.tag! 'n2:HandlingTotal', localized_amount(options[:handling]),'currencyID' => currency_code
                  xml.tag! 'n2:TaxTotal', localized_amount(options[:tax]), 'currencyID' => currency_code
                end
            ...
            xml.tag! 'n2:CreateRecurringPaymentsProfileRequestDetails' do
              ...
              xml.tag! 'n2:ScheduleDetails' do
                ...
                xml.tag! 'n2:PaymentPeriod' do
                  ...
                  xml.tag! 'n2:Amount', localized_amount(options[:amount]), 'currencyID' => options[:currency] || 'USD'
                  xml.tag! 'n2:TaxAmount', localized_amount(options[:tax_amount] || 0), 'currencyID' => options[:currency] || 'USD'
                end
                if !options[:trialamount].blank?
                  xml.tag! 'n2:TrialPeriod' do
                    ...
                    xml.tag! 'n2:Amount', localized_amount(options[:trialamount]), 'currencyID' => options[:currency] || 'USD'
                  end
                end
                if !options[:initial_amount].blank?
                  xml.tag! 'n2:ActivationDetails' do
                    xml.tag! 'n2:InitialAmount', localized_amount(options[:initial_amount]), 'currencyID' => options[:currency] || 'USD'
